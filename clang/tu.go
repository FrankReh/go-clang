...
func (tu TranslationUnit) IsFileMultipleIncludeGuarded(file File) bool {
	return C.clang_isFileMultipleIncludeGuarded(tu.c, file.c) != 0
}
...
// can be mapped to a specific entity within the abstract syntax tree. This
// token-annotation routine is equivalent to invoking clang_getCursor() for the
...
// are accepted. For example, given a function call f(x), clang_getCursor()
